...
Before touching any data — real or simulated — a definition is needed.   
I settled on: a picker slot is wasted when it's running below 40% utilisation, meaning the picker is idle for more than 36 minutes of every hour. Below that threshold, the buffer value of keeping the slot active doesn't justify the cost.
...
|---|---|
| `01_sql_schema_queries.sql` | How to structure and query the underlying store data |
| `02_demand_forecast.ipynb` | How bad is the waste? What will demand look like next week? |
| `02_7day_forecast.csv` | Forecast output, ready for the dashboard or WMS to consume |
| `02_demand_forecast_charts.png` | Visual proof of model accuracy and demand patterns |
| `03_PRD.md` | What's being built, why, and how to know it worked |
| `04_AB_test_design.md` | How to prove the ML restocking trigger is better than manual |
