-- ============================================================
-- Uber India — EV Transition: Driver Cohort & Scoring Query
-- Deliverable 1: EV-eligible driver targeting
-- ============================================================
-- Purpose:
...
--         incentive budget allocation (feeds Deliverable 2)
-- ============================================================
...
scored AS (
  SELECT
    ...
    --   Short trips + city-centre density = well within EV range,
    --   near likely charging infrastructure. Gap vs conversion = suppressed intent.
    avg_trip_dist_km  <= 12
    ...
    --   Long-tenured driver on an aging vehicle without upgrading =
    --   capital barrier, not intent barrier. Loan tie-up is the unlock.
    tenure_months     >= 24
...
--
-- Note: COUNTIF() is BigQuery syntax.
--   Postgres equivalent: COUNT(CASE WHEN <condition> THEN 1 END)
--   DATEDIFF() behaviour varies by dialect — validate against your warehouse.
-- ============================================================
